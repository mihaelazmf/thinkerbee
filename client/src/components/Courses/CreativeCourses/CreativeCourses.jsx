...
const CreativeCourses = ({ createdCourse }) => {
    ...
    const fetchCourses = async () => {
      try {
        const response = await axios.get(
          "http://localhost:3001/courses?category=Creative"
        );
  ...
  const filteredCourses = courses.filter(
    (course) => course.category === "creative"
  );
  ...
  return (
    ...
    >
      ...
      {filteredCourses.length > 0 ? (
        ...
        <p className="text-center">No creative courses available.</p>
      )}
    </div>
  );
};
